...
export async function createUser(userData) {
  ...
  const [result] = await db.insert(table.user)
    .values(user)
    .returning({
      id: table.user.id,
      username: table.user.username,
...
export async function updateUserProfile(userId, userData) {
  const [user] = await db.update(table.user)
    .set(userData)
...
export async function getDepartmentById(id) {
  const [department] = await db.select()
    .from(table.department)
    .where(eq(table.department.id, id));
...
export async function createDepartment(name, description) {
  const [department] = await db.insert(table.department)
...
export async function createRole(name, permissions) {
  const [role] = await db.insert(table.role)
